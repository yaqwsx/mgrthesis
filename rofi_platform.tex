...
is implemented as a simple UART (Universal Synchronous Receiver and
Transmitter).
...
\begin{bytefield}{32}
    ...
    \bitbox{32}{\textit{padding}} \\
    \wordbox[tb]{2}{pause at least 8 bytes (no clock)}  \\
    \wordbox{2}{dock response/extended data}
\end{bytefield}

\noindent Command data (including the command itself) are padded by zeroes to 32
bits. The dock response requires no padding. The commands 0--127 are reserved
for the base dock, and all dock types are required to support them. The commands
128--255 are dock type specific. The basic commands are:
...
\paragraph{Send blob (3)} is used to send a binary blob. The extended data are
