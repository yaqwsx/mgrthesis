...
much more complicated. The dedicated sense pin can detect if the mating site is
still attached.
...
2 high (bot over a protection resistor) as shown in the figure
...
frame after the silent period and prepare for next incoming frame.

The hardware can implement baudrate correction algorithms and leverage the
constant header 0xAA for this purpose.
...
line. We choose 48 V line to mitigate the power on the conductors.

The dock exposes two power lines to the module -- \emph{EXT} and \emph{INT}. The
EXT line is required to be directly connected all EXT lines of the other docks
in a module. The purpose of the EXT line is to share power between connected
devices. The INT line is supposed to power the module and e.g. charge an
internal accumulator. See example of dock connection in the universal module in
the figure \ref{fig:um_internal}. Both of these lines can be connected or
disconnected independently to the source. The hardware implementation of the
switch is shown in the figure \ref{fig:dock_biswitch}. Simple transistor switch
is not enough as the current can flow both direction. The switch also
...
This separation of the power lines into the EXT and INT lines allow for
...
\begin{enumerate}
    ...
    the system into several independent power sharing parts.
...
interrupt to the master -- when interrupt is issued, the pin is pulled low.
Words are transmitted in the little endian byte order, bytes are sent
...
There can be multiple variants of the docks (featuring sensors). So far, we
define only dock variant 1 (with no sensors) further referenced as a \emph{base
dock}. The following text describes revision 1 of the communication protocol,
future revisions might be released. Future revisions must be
backward-compatible.
...
transactions must have no visible effect. Each transaction follows the format:
...
No padding of the command data or dock response is required. The commands 0--127
are reserved for the base dock and all dock types are required to support them.
The commands 128-255 are dock type specific. The basic commands are:
...
The dock response follows similar format as the command. The dock returns the
...
Bits set to 1 signal interrupt reason. After the response is send, all interrupt
reasons are cleared. Therefore, the mask returns all interrupts reasons from
last query.

\paragraph{Send blob (3)}

\paragraph{Receive blob (4)}
