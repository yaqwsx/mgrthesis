...
Consider a single cell sphere, the basic abstract building block of modules
(figure \ref{sec:dock_in_grid}). We define origin of the cell in the middle of
the sphere. There up to 6 adjacent cells of the grid (in both direction of all
axis X, Y and Z.) This defines 6 possible dock positions which we name after an
axe direction: +X, -X, +Y, -Y, +Z, -Z. We also give an orientation of the dock
(see figure \ref{sec:dock_in_grid}). The convention for the orientation is that
it follows direction of the previous axe.
...
system which is essential for any reconfiguration.
