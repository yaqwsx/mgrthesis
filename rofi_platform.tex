...
The platform should allow distributed control as distributed environment can be
easily converted a centrally converter, however the vise-versa is not easily
possible.
...
the platform, one module should be able to disconnect without cooperation of
...
\begin{enumerate}
    ...
    \item an intermodule communication (section \ref{sec:communication}) and
...
their inscribed spheres (see double cell module in figure
\ref{fig:rofi_shapes}). The module hull is then defined as union of all such
existing convex hulls for the module. See figure \ref{fig:rofi_shapes} for an
example. When we will reference a module shape, we use following terminology:
\begin{enumerate*}
...
We assume the modules are physically able to perform movement with practically
unlimited granularity (considering limits of a mechanic construction). However,
the reconfiguration algorithms built on top of the system can consider only a
...
slide under hooks of the clip from mating side and form a connection which
...
``unscrew''). As the clip moves upwards it carries the skirt. The skirt moves in
a slot which prevents it from rotating against body and therefore it only slides
upwards. So far, the clip is extended to a locking distance, however, the hooks
of the docks could not slide under each other. The helical slot is ended by a
horizontal part (see figure \ref{fig:dock_key_components}). This allows for
...
position by an active component (e.g. motor) and therefore energy is only
...
hooks slides under each other and finish the locking process (situation 4).
...
was achieved. Flat dock profile is a great advantage -- there is more much
...
the design is commonly available ``Pololu 298:1 Micro Metal Gearmotor HPCB
...
one of the pad sections 1 or 2. There are 5 pins in total; two pins for power
...
much more complicated. The dedicated sense pin can also easily detect if the
...
be determined by table in figure \ref{fig:dock_pins}.

The docks communicate over UART with 8 bits, no parity and a 1.5 stop bits at 6
...
The frame are transmitted to the mating dock over UART. If the mating dock is
...
can implement baudrate correction algorithms and leverage the constant header
...
line. We choose 48 V line to mitigate the power losses on the conductors.
...
connected devices. The INT line is supposed to power the module and e.g. charge
an internal accumulator. See example of dock connection in the universal module
in the figure \ref{fig:um_internal}. Both of these lines can be connected to or
disconnected from the source independently. The hardware implementation of the
switch is shown in the figure \ref{fig:dock_biswitch}. Simple transistor switch
is not enough as the current can flow both direction. The switch also monitors
line voltage and measures current via electromagnetic coupling.
...
\begin{enumerate}
    \item The module does not participate on power sharing (all switches are
...
dock (figure \ref{fig:dock_block}). The chip select pin (CS) serves also as an
interrupt to the master -- when interrupt is issued, the pin is pulled low by
the slave. Words are transmitted in the little endian byte order, bytes are sent
most-significat bit first. The data are read at the rising edge of the clock
...
The variants are assigned unique number identifier. So far, we specify only dock
variant 1 with no sensors, further referenced as a \emph{base dock}. The
following text describes revision 1 of the communication protocol, future
revisions might be released. Future revisions must be backward-compatible.

The communication on the bus is performed in transactions -- each transactions
...
\paragraph{Version command (0):} has no command data. The response is in format:
...
\begin{itemize}
    ...
    \item E (external, 1-bit) -- 1: the EXT line is connected, 0: the line is diconnected,
...
setting a corresponding bit in write mask, we can change state of the
corresponding feature of the dock to the state specifed by status bitmask. E.g.
by setting bit I to 1 and setting corresponding bit in write mask, the INT line
will be connected. Zero bits in the write bitmask are left unchanged. The format
...
\noindent The dock response follows similar format as the command. The dock returns the
status before performing the command. It also sends number of blobs waiting in
the buffer for transmission, number of blobs not retrieved by the master and
voltage and current on the power lines. The voltage in 8.8 fixed point number
in volts, the current is in 8.8 fixed point number in Amperes. The format is
following:
...
\begin{itemize}
    \item C -- connect or disconnect event occured (the sense pin change) and
...
\noindent Bits set to 1 signal interrupt reason. After the response is send, all
interrupt reasons are cleared. Therefore, the mask returns all interrupts
reasons from last query.
...
dock. The oldest received blob is send. The command has no data. The response
follows the format:
...
\noindent If there are no pending blobs, blob of length zero is returned.
...
\ref{fig:dock_positions}). We define origin of the shoe in the middle of the
...
also give an orientation of the dock (see figure \ref{fig:dock_positions}).
...
Given joint position, the shape descriptors allow for an easy computation of
